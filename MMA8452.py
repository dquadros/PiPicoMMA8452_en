'''
MicroPython class to interface with a MMA8452 sensor
(C) 2021, Daniel Quadros

Adaptation of the Sparkfun library for Arduino:
...
class MMA8452():
    ...
    # Returns raw acceleration
    # format in registers hhhhhhhh llll0000
    # stuct.unpack(">h", x)  means x has a 16 bits number with most significant byte first
    def getRawAcel(self):
        acel = struct.unpack(">hhh", self.readRegisters (self.OUT_X_MSB, 6))
        return [a >> 4 for a in acel]

    # Returns scaled acceleration
    def getCalclulatedAcel(self):
        return [float(a)/float(1 << 11)*float(self.scale) for a in self.getRawAcel()]
    
    # Checks if new info available
    def available(self):
    ...
    def setScale(self, fsr):
        ...
        # Must be in standby mode to change scale
        if self.isActive():
            self.standby()
    
        # Calculate new register value and update
    ...
    def setDataRate(self, odr):
        if not odr in (self.ODR_800, self.ODR_400, self.ODR_200, self.ODR_100,
            ...
            raise ValueError("Invalid value for data rate")

        # Must be in standby mode to change
        if self.isActive():
            self.standby()
    ...
    # Configure Portrait/Landscape detection
    def setupPL(self):
        # Must be in standby mode to change
        if self.isActive():
            self.standby()
...
if __name__ == "__main__":
    '''
        Quick test.
        Assumes the sensor has the default address and is connected to default i2c0 pins
    '''
    ...
    if not sensor.begin():
        ...
        while True:
            ...
            if newpos == sensor.LOCKOUT:
                ...
                print ("Landscape turned to right")
            elif newpos == sensor.LANDSCAPE_L:
                print ("Landscape turned to left")
            elif newpos == sensor.PORTRAIT_U:
                print ("Portrait turned up")
            elif newpos == sensor.PORTRAIT_D:
                print ("Portrait turned down")
            else:
                print ("Invalid direction: "+str(newpos))
